...
class libraryCards {
    ...
    fade(card) {
        ...
        // Card flips over.
        if (card.isValid()) {
            // The new image path will be the path for the front face
            // of the card.
            let newSrc = card.imgValue;  // card.imgValue contains the path for the front face.

            // Fade the existing card out
            card.imgObject.fadeOut(500, function() {
                // This callback runs when the existing card is all
                // the way faded, so now update the src to the new location,
                // amd fade in.
